...
test('promotion은 verified CI repository와 digest에 Dev image를 결속한다', () => {
  const digest = `sha256:${'d'.repeat(64)}`;
  const dev = updateDeliveryImages(source, 'dev.example/sample-app', digest);

  assert.throws(
    () => promoteDeliveryImages(dev, source),
    /verified CI image is required/,
  );
  ...
  assert.throws(
    ...
    /Dev repository does not match verified CI/,
  );
  assert.throws(
    () => promoteDeliveryImages(dev, source, {
      repository: 'dev.example/sample-app',
    ...
    /Dev digest does not match verified CI/,
  );
});
