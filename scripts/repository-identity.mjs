...
function requireCanonicalRepositoryId(value) {
  const repositoryId = normalizeRepositoryId(value);
  if (repositoryId !== canonicalRepositoryId) throw new Error('REPOSITORY_ID_MISMATCH');
  return repositoryId;
}

function requireCanonicalRepositoryContext(value) {
  return requireCanonicalRepositoryId(String(value));
}

export function resolveExpectedRepositoryId({ expectedRepositoryId, workflowRun } = {}) {
  if (expectedRepositoryId !== undefined) requireCanonicalRepositoryContext(expectedRepositoryId);
  if (workflowRun?.repository?.id !== undefined) {
    requireCanonicalRepositoryContext(workflowRun.repository.id);
  }
...
export function assertRepositoryIdentity({ repositoryId, repositoryName, workflowRun, expectedRepositoryId }) {
  const actualId = requireCanonicalRepositoryId(repositoryId);
  ...
  if (workflowRun) {
    const runRepositoryId = requireCanonicalRepositoryContext(workflowRun.repository?.id ?? '');
